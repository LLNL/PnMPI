                       ...
                       ...)
{
  ...
  /* Create a new buffer for a format containing function and line parameters.
   * Separated prints after printing the format string can't be used as the
   * buffer will be flushed after calling pnmpi_print_prefix_rank, so function
   * and line may be attached to a different message from an other rank. */
  size_t len = strlen(function) + floor(log10(abs(line))) + strlen(format) + 5;
  char buffer[len];
  if (snprintf(buffer, len, "%s:%d: %s", function, line, format) >= len)
    {
