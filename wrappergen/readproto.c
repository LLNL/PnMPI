  ...
  *fn_list = (fn_def *)malloc( sizeof( fn_def ) * *n_fn );
  ...
  for (i=0; i < *n_fn; i++) {
    ...
    if (IsNameInList( ptr, fn_names, n_fn_names )) {
      ...
      (*fn_list)[fn_num].fortranname = malloc( strlen( ptr ) + 2 );
      sprintf((*fn_list)[fn_num].fortranname,"%s_",ptr);
      StrToLower( (*fn_list)[fn_num].fortranname );
      
      (*fn_list)[fn_num].namepp = malloc( strlen( ptr ) + 1 );
      sprintf((*fn_list)[fn_num].namepp,"%s",ptr);
      if (strlen((*fn_list)[fn_num].namepp)>4)
	{
	  (*fn_list)[fn_num].namepp[0]++;
	  (*fn_list)[fn_num].namepp[1]++;
	  (*fn_list)[fn_num].namepp[2]++;
	}
      else
	{
	  fprintf(stderr,"ERROR: NAME DOES NOT START WITH MPI_\n");
	  exit(1);
	}
	...
	(char **)malloc( (*fn_list)[fn_num].nargs * sizeof( char * ) );
      (*fn_list)[fn_num].argPointers =
	(int *)malloc( (*fn_list)[fn_num].nargs * sizeof( int ) );
	...
	if ((strchr((*fn_list)[fn_num].argTypePrefix[argnum],'*')!=NULL) ||
	    (strchr((*fn_list)[fn_num].argTypeSuffix[argnum],'[')!=NULL) )
	  {
	    (*fn_list)[fn_num].argPointers[argnum]=1;
	  }
	else
	  {
	    (*fn_list)[fn_num].argPointers[argnum]=0;
	  }

	#if DEBUG>1
	fprintf( stderr, "Arg %d: %s - %s - %s", argnum,
		(*fn_list)[fn_num].argTypePrefix[argnum],
		(*fn_list)[fn_num].argNames[argnum],
		(*fn_list)[fn_num].argTypeSuffix[argnum] );
	if ((*fn_list)[fn_num].argPointers[argnum])
	  printf("# YES\n");
	else
	  printf("# NO\n");
	#endif
