...
replacement string.\n", finfo->name, startingLine );
  ...
  } else if (!strcmp( escBodyList[0], "forallfortranfn" )) {
    if (escBodyLen>1) {
      /* if a variable name was found (syntax check) */
      if (ReadUntilMatch( finfo, "forallfortranfn",
			  "endforallfortranfn", &body, startingLine ))
	DoForFortranAll( outf, finfo, rinfo, escBodyList, escBodyLen, escBody,
			startingLine, body );
    } else { /* if valid syntax */
      fprintf( stderr, "(file %s, line %d) forallfn needs a function name \
replacement string.\n", finfo->name, startingLine );
  ...
  } else if (!strcmp( escBodyList[0], "forallfnpp" )) {
      if (escBodyLen>1) {
      /* if a variable name was found (syntax check) */
      if (ReadUntilMatch( finfo, "forallfnpp",
			  "endforallfnpp", &body, startingLine ))
	DoForAllPP( outf, finfo, rinfo, escBodyList, escBodyLen, escBody,
			startingLine, body );
    } else { /* if valid syntax */
      fprintf( stderr, "(file %s, line %d) forallfn needs a function name \
replacement string.\n", finfo->name, startingLine );
...
replacement string and at least one function name.\n",
  ...
  } else if (!strcmp( escBodyList[0], "last_num" )) {
      fprintf( outf, "%d", fn_num-1 );
  } else if (!strcmp( escBodyList[0], "retType" )) {
      fprintf( outf, "%s", rinfo->fn_list[rinfo->active_fn].returnType );
  } else if (!strncmp( escBodyList[0], "MACRO_",6)) {
    char *_tn=&(escBodyList[0][6]);
    int _i;
    for (_i=0; _i<rinfo->fn_list[rinfo->active_fn].nargs; _i++)
      {
	if (!strcmp(rinfo->fn_list[rinfo->active_fn].argTypePrefix[_i],_tn))
	{
	  fprintf(outf,"%s(%s);\n",escBodyList[0],
		  rinfo->fn_list[rinfo->active_fn].argNames[_i]);
	}
      }
  } else if (!strncmp( escBodyList[0], "MACRO1_",7)) {
    char *_tn=&(escBodyList[0][6]);
    int _i;
    for (_i=0; _i<rinfo->fn_list[rinfo->active_fn].nargs; _i++)
      {
  ...
  } else if (!strncmp( escBodyList[0], "MACRO2_",7)) {
    char *_tn=&(escBodyList[0][6]);
    int _i;
    for (_i=0; _i<rinfo->fn_list[rinfo->active_fn].nargs; _i++)
      {
  ...
  } else if (!strcmp( escBodyList[0], "argList" )) {
    int _i;
    fprintf(outf,"(");
    for (_i=0; _i<rinfo->fn_list[rinfo->active_fn].nargs; _i++)
      {
	if (_i!=0)
	  fprintf( outf, ", ");
	fprintf( outf, "%s", rinfo->fn_list[rinfo->active_fn].argNames[_i] );
      }
    fprintf(outf,")");
  } else if (!strcmp( escBodyList[0], "argListOpen" )) {
    int _i;
    for (_i=0; _i<rinfo->fn_list[rinfo->active_fn].nargs; _i++)
      {
        fprintf( outf, ", ");
  ...
  } else if (!strcmp( escBodyList[0], "argListDeref" )) {
    int _i;
    fprintf(outf,"(");
    for (_i=0; _i<rinfo->fn_list[rinfo->active_fn].nargs; _i++)
      {
	if (_i!=0)
	  fprintf( outf, ", ");
	if (rinfo->fn_list[rinfo->active_fn].argPointers[_i])
	  {
	    fprintf( outf, "%s", rinfo->fn_list[rinfo->active_fn].argNames[_i] );
	  }
	else
    ...
    fprintf(outf,")");
  } else if (!strcmp( escBodyList[0], "argTypeList" )) {
    int _i;
    fprintf(outf,"(");
    for (_i=0; _i<rinfo->fn_list[rinfo->active_fn].nargs; _i++)
      {
	if (_i!=0)
	  fprintf( outf, ", ");
	fprintf( outf, "%s %s%s", 
		 rinfo->fn_list[rinfo->active_fn].argTypePrefix[_i],
		 rinfo->fn_list[rinfo->active_fn].argNames[_i],
		 rinfo->fn_list[rinfo->active_fn].argTypeSuffix[_i] );
      }
    fprintf(outf,")");
  } else if (!strcmp( escBodyList[0], "argTypeListVoid" )) {
    int _i;
    fprintf(outf,"(");
    for (_i=0; _i<rinfo->fn_list[rinfo->active_fn].nargs; _i++)
      {
    ...
    if (rinfo->fn_list[rinfo->active_fn].nargs==0)
      fprintf(outf,"void");
    fprintf(outf,")");
  } else if (!strcmp( escBodyList[0], "argTypeListOpen" )) {
    int _i;
    for (_i=0; _i<rinfo->fn_list[rinfo->active_fn].nargs; _i++)
      {
	fprintf( outf, ", ");
  ...
  } else if (!strcmp( escBodyList[0], "argTypeListPtr" )) {
    int _i;
    fprintf(outf,"(");
    for (_i=0; _i<rinfo->fn_list[rinfo->active_fn].nargs; _i++)
      {
...
void DoForFortranAll( outf, finfo, rinfo, bodyList, bodyLen, escBody, startLine,
		      body )
FILE *outf;
fileinfo *finfo;
rpcinfo *rinfo;
...
int  bodyLen, startLine;
{
  ...
  rpc1.from = bodyList[1];
  rpc2.from = bodyList[2];
  my_finfo.name = finfo->name;
  my_finfo.filenum = finfo->filenum;
  ...
  for (fn_num=0; fn_num < rinfo->n_fn; fn_num++) {
    ...
    if (!IsNameInList( rinfo->fn_list[fn_num].name, bodyList+2, bodyLen-2 )) {
      my_finfo.lineno = startLine;
      my_finfo.str = body;
      rpc1.to = rinfo->fn_list[fn_num].fortranname;
      rpc2.to = rinfo->fn_list[fn_num].name;
      ListAddItem( rinfo->rpc, replacement, rpc1 );
      ListAddItem( rinfo->rpc, replacement, rpc2 );
      rinfo->active_fn=fn_num;
#if DEBUG
      fprintf( stderr, "foreach replacement of %s with %s.\n",
	      bodyList[1], rinfo->fn_list[fn_num].fortranname );
      fprintf( stderr, "foreach replacement of %s with %s.\n",
	      bodyList[2], rinfo->fn_list[fn_num].name );
#endif
...
void DoForAllPP( outf, finfo, rinfo, bodyList, bodyLen, escBody, startLine,
		 body )
FILE *outf;
fileinfo *finfo;
rpcinfo *rinfo;
...
int  bodyLen, startLine;
{
  ...
  for (fn_num=0; fn_num < rinfo->n_fn; fn_num++) {
    ...
    if (!IsNameInList( rinfo->fn_list[fn_num].name, bodyList+2, bodyLen-2 )) {
      my_finfo.lineno = startLine;
      my_finfo.str = body;
      rpc1.to = rinfo->fn_list[fn_num].namepp;
      rpc2.to = rinfo->fn_list[fn_num].name;
      ListAddItem( rinfo->rpc, replacement, rpc1 );
      ListAddItem( rinfo->rpc, replacement, rpc2 );
      rinfo->active_fn=fn_num;
#if DEBUG
      fprintf( stderr, "foreach replacement of %s with %s.\n",
	      bodyList[1], rinfo->fn_list[fn_num].namepp );
      fprintf( stderr, "foreach replacement of %s with %s.\n",
	      bodyList[2], rinfo->fn_list[fn_num].name );
#endif
